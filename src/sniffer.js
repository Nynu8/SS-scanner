...
module.exports.sniff = (settings) => {
  ...
  let filter = `host ${settings.serverAddress}`;
  let bufSize = 10 * 1024 * 1024;
  let buffer = Buffer.alloc(65535);

  try {
    let linkType = capturer.open(
      Cap.findDevice(settings.networkInterface),
      filter,
      bufSize,
      buffer
    );
        ...
        if (ret.info.type === PROTOCOL.ETHERNET.IPV4) {
          ...
          if (ret.info.protocol === PROTOCOL.IP.TCP) {
            ...
            if (data.includes(`${eventChatPrefix}Entering`)) {
              ...
              if (solarSystem !== null) {
                if (solarSystem.numberOfBodies() > 0) {
                  if (await SolarSystem.alreadyScanned(systemName)) {
                    if (
                      solarSystem.numberOfBodies() <
                      (await SolarSystem.amountScanned(systemName))
                    ) {
                      console.log(
                        chalk`{blue Number of bodies scanned is {cyan.underline smaller} than already saved: {red Overwrite cancelled}}`
                      );
                    } else {
                      console.log(
                        chalk`{blue Number of bodies scanned is {cyan.underline bigger or equal} to already saved: {green Overwriting}}`
                      );
                  ...
                  console.log(
                    chalk`{blue Nothing scanned, skipping {yellowBright.bold ${systemName}}}`
                  );
                }
              }
              ...
              if (await SolarSystem.alreadyScanned(systemName)) {
                console.log(
                  chalk`{blue {yellowBright ${systemName}} was already scanned and contains {yellowBright ${await SolarSystem.amountScanned(
                    systemName
                  )}} entries}`
                );
              }
            ...
            if (
              data.includes(`${eventChatPrefix}Scan:`) &&
              !data.includes(`No base slots for extraction`)
            ) {
              let msg = getFullString(data, data.indexOf("Scan:"));
              if (solarSystem === null) {
                console.log(
                  chalk.red(
                    "I don't know where you are! Please warp to a galaxy to start scanning"
                  )
                );
              } else {
                ...
                if (solarSystem.alreadyScanned(solarbody.name)) {
                  console.log(
                    chalk`{yellowBright.bold ${solarbody.name}} {blue was already scanned}`
                  );
                } else {
                  solarSystem.addSolarbody(solarbody);
                  solarSystem.addRawSolarbody(msg);
                  console.log(
                    chalk`{yellowBright.bold ${solarbody.name}} {blue was added to the list}`
                  );
                }
              }
            }
    ...
    console.log(
      chalk.bgRed.yellowBright.bold(
        "Failed to open your network interface. Please delete your config.json file and restart the program!"
      )
    );
  }
};
...
module.exports.findCorrectInterface = async () => {
  ...
  console.log(
    chalk.bgBlue.greenBright("-----Looking for correct network adapter-----")
  );
  for (let i = 0; i < interfaceCount; i++) {
    for (let j = 0; j < interfaceList[i].addresses.length; j++) {
      let interfaceTested = Cap.deviceList()[i].addresses[j].addr;
      console.log(
        chalk`{cyan Testing address {yellowBright.bold ${interfaceTested}} on interface {yellowBright.bold ${i}}}`
      );
...
async function testInterface(interface) {
  ...
  try {
    linkType = capturer.open(
      Cap.findDevice(interface),
      filter,
      bufSize,
      buffer
    );
