...
module.exports.parseScan = function(scanString) {
  ...
  let metals = scanString.match(
    "(?:A )?(smidgin|little|bit|bunch|lot|Plenty|Loads) (?:of )?Metals"
  );
  if (metals !== null) metals = metals[1];

  let nukes = scanString.match(
    "(?:A )?(smidgin|little|bit|bunch|lot|Plenty|Loads) (?:of )?Nuclear Waste"
  );
  if (nukes !== null) nukes = nukes[1];

  let silicon = scanString.match(
    "(?:A )?(smidgin|little|bit|bunch|lot|Plenty|Loads) (?:of )?Silicon"
  );
  if (silicon !== null) silicon = silicon[1];

  let spaceOats = scanString.match(
    "(?:A )?(smidgin|little|bit|bunch|lot|Plenty|Loads) (?:of )?Space Oats"
  );
